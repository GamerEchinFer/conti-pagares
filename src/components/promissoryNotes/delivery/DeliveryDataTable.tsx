...
const DeliveryDataTable = memo(({ data, isLoading, columns, nestedFields }: DeliveryDataTableProps) => {
                ...
                if (typeof updaterOrValue === "function") {
                    ...
                    const subRowsSelection = data
                        ...
                        ?.subRows?.reduce((acc, curr) => {
                            acc[`${curr.operacion}_${curr.cliente.codigoCliente.toString()}`] = codClientSelectedValue;
                            return acc;
                        }, {} as SimpleRowSelectionState);
                    setSubRowSelection(subRowsSelection ?? {});
                    return newValue;
    ...
    const handleChangeSubRowSelection = (
        ...
        setSubRowSelection((old) => {
            if (typeof updaterOrValue == "function") {
                ...
                const codClientSelected = newSelectedKey.length > 1 ? newSelectedKey[1].split("_")[1] : "";

                const existCodClientSelected = keysSubRowSelectionMapped.some(
                    key => key == codClientSelected && key != parentRow.original.cliente.codigoCliente.toString()
                );
                if (existCodClientSelected) {
                    alert("No se puede seleccionar un hijo de otro padre");
                    return old;
                }
    ...
    const handlerClickDelivery = () => {
        ...
        const promissoryNotesSelected = data?.filter(d => {
            const keysSubRowSelection = Object.keys(subRowSelection);
            const keysSubRowSelectionMapped = keysSubRowSelection.map(key => key.split("_")[1]);
            const someSubRowSelected = keysSubRowSelectionMapped.some(key => key == d.cliente.codigoCliente.toString());
    ...
    return (
        <>
            ...
            <CustomMaterialDataTable
                ...
                nestedTableProps={{
                    getRowId: (row: PromissoryNotesConsultDelivery) => `${row.operacion}_${row.cliente.codigoCliente}_${row.cuota}`,
                }}
